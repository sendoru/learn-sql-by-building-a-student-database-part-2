...
echo "$($PSQL \
        "\
          ...
          WHERE major is not null and (first_name ilike '%ma%' or student_id is null)\
          ORDER BY major\
        "\
      )"
...
echo "$($PSQL \
        "\
          ...
          FROM students FULL JOIN majors using(major_id)\
                    FULL JOIN majors_courses using(major_id)\
                    FULL JOIN courses using(course_id)\
          WHERE student_id is null or (first_name like 'Obie' and last_name like 'Hilpert')\
          ORDER BY course DESC\
        "\
      )"
